...
class Board:
            ...
            if prev == (-1, -1):
                ...
                if self.board[prev[0]][prev[1]].color != self.board[row][col].color:
                    ...
                    if self.board[row][col].color == color:
                        ...
                        self.reset_selected()
                        if (
                            self.board[prev[0]][prev[1]].moved == False
    ...
    def move(self, start, end, color):
        ...
        self.board = nBoard[:]
        checkedAfter = self.is_checked(color)
        if checkedAfter:
            changed = False
    ...
    def stalemate(self, color):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                if self.board[i][j] != 0:
                    if self.board[i][j].color == color:
                        moves = self.board[i][j].move_list
                        if moves:
                            return False
    ...
    def promote_pawn(self, row, col):
        ...
        if piece.pawn and ((piece.color == "w" and row == 0) or (piece.color == "b" and row == 7)):
    ...
    def get_remaining_time(self, color):
        ...
        if color == "w":
            remaining_time = max(0, self.time1 - elapsed_time + self.storedTime1)
        else:
            remaining_time = max(0, self.time2 - elapsed_time + self.storedTime2)
